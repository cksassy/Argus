...
public class HeimdallTotalAvaTransform implements Transform {
	...
	public List<Metric> transform(List<Metric> metrics, List<String> constants) {
		...
		final List<Metric> metricsAPTTime = metricsLineup.stream()
				.filter(m -> Pattern.matches(KafkaPattern.APT_TIME.toString(), m.getMetric()))
				.collect(Collectors.toList());
		final List<Metric> metricsAPTCount = metricsLineup.stream()
				.filter(m -> Pattern.matches(KafkaPattern.APT_COUNT.toString(), m.getMetric()))
				.collect(Collectors.toList());
		final List<Metric> metricsCPU = metricsLineup.stream()
				.filter(m -> Pattern.matches(KafkaPattern.CPU.toString(), m.getMetric())).collect(Collectors.toList());
		...
		ZEROMETRIC.get(0).setTags(getRacServerName(metricsLineup));
	...
	public List<Metric> transform(List<Metric> metrics) {
		...
		final List<Metric> ZEROMETRICHour = downsample("1h-avg", ZEROMETRIC);

		List<Metric> averagedAPT = getRacLevelAPT(ZEROMETRIC, metricsAPTTime, metricsAPTCount);
		List<Metric> impactedMin = fillWithTemplate(ZEROMETRICHour, getImpactedMin(averagedAPT, metricsCPU));
		...
		List<Metric> avarate = fillWithTemplate(ZEROMETRICHour,divide(reduceMetricListSafe(impactedMin, cull_below(denominator, 0))));
		Map<Long, String> tsTotalAvaRate = (avarate).get(0).getDatapoints();

		List<Metric> cull_below_metricsCPU = cull_below(metricsCPU, 65);
		List<Metric> cpu = fillWithTemplate(ZEROMETRICHour, downsample("1h-count", cull_below_metricsCPU));
	...
	private final Map<String, String> getRacServerName(List<Metric> metrics) {
		String scopeAddress=(metrics.get(0).getScope());//core.CHI.SP1.na11
		String metricAddress=(metrics.get(0).getMetric());
		String[] scopeAddressList = scopeAddress.trim().split("\\.");

		String racName=metricAddress.split("\\.")[0];
		String address=scopeAddressList[1]+"."+scopeAddressList[2]+"."+scopeAddressList[3]+"."+(racName.substring(racName.indexOf("Time")+4));
	...
	private final List<Metric> getRacLevelAPT(List<Metric> template, List<Metric> appLevelApt,
			List<Metric> appLevelCount) {
		List<Metric> m_product = fillWithTemplate(template,
				scale_match(reduceMetricListSafe(appLevelApt, appLevelCount)));
	...
	private final List<Metric> constructingReturn(List<Metric> template, Map<Long, String> tsTotalAvaRate, Map<Long, String> tsImpactMins, Map<Long, String> tsCPU) {
		...
		newMetricsList.add(generateMetric(distiller, tsImpactMins, "TTM"));
		newMetricsList.add(generateMetric(distiller, tsCPU, "CPU Impacts"));
		return newMetricsList;
	}
	
	private final List<Metric> constructingReturn(List<Metric> template, Map<Long, String> averagedAPT, Map<Long, String> tsCPU) {
		List<Metric> newMetricsList = new ArrayList<Metric>();
		MetricDistiller distiller = new MetricDistiller();
		distiller.distill(template);
		newMetricsList.add(generateMetric(distiller, averagedAPT, "trafic-weighted avg APT"));
		newMetricsList.add(generateMetric(distiller, tsCPU, "CPU utlization"));
		return newMetricsList;
	}
	...
	private final List<Metric> fillWithTemplate(List<Metric> template, List<Metric> m) {
		...
		assert (outdatapoints.entrySet().stream().filter(k -> !template.get(0).getDatapoints().keySet().contains(k))
				.collect(Collectors.toSet()).size() == 0) : "The newTS time series has to be a subset of template";
		template.get(0).getDatapoints().entrySet()
				.forEach(e -> outdatapoints.putIfAbsent(e.getKey(), String.valueOf(0)));
